...
<a href="https://pronounce.renlab.ai/browse.html"><img src="https://img.shields.io/badge/dictionary-1548%20entries-8fd694" alt="Entries"></a>
...
Hover over any tech word in any file — see the IPA, hear the pronunciation. Same 1548-entry dictionary as the CLI, JSON-bundled at build (zero runtime parse cost).
...
- **`Pronounce: Search dictionary…`** — fuzzy-find all 1548 entries.
...
Click any tech word on any webpage → popup with IPA + audio. Same 1548-entry dictionary; same Web Speech API as `pronounce.renlab.ai`. **Sideload only for now** (not yet on Chrome Web Store).
...
`data/pronunciations.tsv` is the single source of truth — tab-separated, 1548 entries, covering:
