...
<a href="https://pronounce.renlab.ai/browse.html"><img src="https://img.shields.io/badge/dictionary-1650%20entries-8fd694" alt="Entries"></a>
...
- **1650 entries, every one sourced.** Confidence-tagged (`creator-clarified` / `community-consensus` / `contested`) with a citable URL where one exists. Wilhite said GIF is "jif" at the 2013 Webby Awards. Crockford says JSON is "JAY-son" (RailsConf 2009). RFC 7519 says JWT is "jot". The dictionary cites them.
...
Hover over any tech word in any file — see the IPA, hear the pronunciation. Same 1650-entry dictionary as the CLI, JSON-bundled at build (zero runtime parse cost).
...
- **`Pronounce: Search dictionary…`** — fuzzy-find all 1650 entries.
...
Click any tech word on any webpage → popup with IPA + audio. Same 1650-entry dictionary; same Web Speech API as `pronounce.renlab.ai`. **Sideload only for now** (not yet on Chrome Web Store).
...
`data/pronunciations.tsv` is the single source of truth — tab-separated, 1650 entries, covering:
