...
# data/entry-dates.tsv — the REAL datePublished/dateModified per entry (committed).
# Must run before anything stamps a page: a new word gets today, a word whose row
# actually changed gets today as `modified`, and every untouched entry keeps its date.
python3 "$REPO_ROOT/tools/build-entry-dates.py"
...
# ---------------------------------------------------------------------------
# sw.js — service worker, EMITTED each build so its cache version tracks content.
# Stamp CACHE_NAME with a hash of the shell (css/js), the dictionary, and this
# build script itself: it changes iff the served content changes, so a new
# deploy makes a new cache, activate() purges the old one, and returning
# visitors stop being pinned to first-cached bytes. Idempotent per commit (same
# inputs → same hash), so it does not create an infinite CI regen loop.
# ---------------------------------------------------------------------------
# Hash the v2 JSX *sources* and the vendored React rather than the built bundle.js:
# the bundle is regenerated later in this same script, so hashing it here would stamp
# the PREVIOUS build's bytes and land every homepage change one deploy late.
SW_STAMP="$(cat "$DOCS/style.css" "$DOCS/script.js" "$DICT" "$REPO_ROOT/tools/build-site.sh" \
  "$DOCS"/v2/*.jsx "$DOCS"/v2/*.css "$DOCS"/v2/vendor/*.js 2>/dev/null | shasum | cut -c1-10)"
cat > "$DOCS/sw.js" <<SWEOF
...
const CACHE_NAME = 'pronounce-${SW_STAMP}';
...
SWEOF
echo "Built $DOCS/sw.js (cache pronounce-${SW_STAMP})"
...
# Join the real per-entry dates onto each row (fields 11 and 12) so the page loop
...
awk -F'\t' -v dates="$REPO_ROOT/data/entry-dates.tsv" '
  BEGIN {
    while ((getline line < dates) > 0) {
      ...
      n = split(line, d, "\t")
      if (n >= 3) { pub[d[1]] = d[2]; mod[d[1]] = d[3] }
    }
    close(dates)
  }
  !/^#/ && NF>=3 && $1 != "" && $1 != "word" {
    ...
    printf "%s\x1f%s\x1f%s\x1f%s\x1f%s\x1f%s\x1f%s\x1f%s\x1f%s\x1f%s\x1f%s\x1f%s\n",
      ...
      (slug in mod ? mod[slug] : ENVIRON["TODAY"])
  }
' "$DICT" > "$TMP_LIST"
...
while IFS="$SEP" read -r word ipa resp alt_ipa alt_resp src_url src_label cat conf notes date_pub date_mod; do
  [[ -z "$word" ]] && continue
...
awk -F'\t' -v dates="$REPO_ROOT/data/entry-dates.tsv" '
  BEGIN {
    ...
    # only set when awk reaches that operand, so BEGIN would see an empty filename.
    while ((getline line < dates) > 0) {
      if (line ~ /^#/ || line == "") continue
      if (split(line, d, "\t") >= 3) mod[d[1]] = d[3]
    }
    close(dates)
  }
...
# ---------------------------------------------------------------------------
# docs/v2/data.js — the homepage's client-side search index. This was NOT wired
# into the build and drifted to 1,212 entries against a 1,790-entry dictionary:
# 578 words (Kokoro, Kiro, the whole JEPA family, …) had a live /word/ page but
# were unfindable from the homepage search box. Regenerate it every build.
# ---------------------------------------------------------------------------
# docs/manifest.webmanifest is hand-maintained and had no emitter, so its PWA
# shortcut sat at "Browse all 310 entries" for ~1,500 entries. Sync it here.
perl -pi -e "s/Browse all [\d,]+ entries/Browse all $ENTRY_COUNT entries/" "$DOCS/manifest.webmanifest"

# docs/v2/bundle.js — the homepage app, esbuild-compiled from the v2 *.jsx sources.
# Same defect build-v2-data.py had: nothing ever ran it, so editing a .jsx silently
# shipped the previous bundle. Rebuild it here.
bash "$REPO_ROOT/tools/build-v2-bundle.sh"
for v in react.production.min.js react-dom.production.min.js; do
  if [[ ! -s "$DOCS/v2/vendor/$v" ]]; then
    echo "build-site: docs/v2/vendor/$v is missing — the homepage self-hosts React." >&2
    exit 1
  fi
done

python3 "$REPO_ROOT/tools/build-v2-data.py"
# Assert the homepage index really covers the dictionary — this is the check that
# would have caught the 1,212-vs-1,790 drift years earlier.
V2_N=$(python3 -c "
import json
s = open('$DOCS/v2/data.js', encoding='utf-8').read()
# Decode from the '[' onward — a regex for the closing ']' trips over brackets
# inside the notes strings.
i = s.index('window.DICT_ALL')
i = s.index('[', i)
print(len(json.JSONDecoder().raw_decode(s, i)[0]))
")
if [[ "$V2_N" -ne "$ENTRY_COUNT" ]]; then
  echo "build-site: docs/v2/data.js indexes $V2_N entries, dictionary has $ENTRY_COUNT." >&2
  exit 1
fi
echo "  v2 search index: $V2_N entries"
...
# ---------------------------------------------------------------------------
# These are load-bearing: build-seo.py emits the collection/compare/zh-word pages
# AND patches sitemap-seo.xml into the sitemap index. Swallowing its failure ships
# a site that is silently missing ~half its indexable pages, so fail the build.
if ! command -v python3 >/dev/null 2>&1; then
  echo "build-site: python3 is required (build-seo.py emits the SEO pages)." >&2
  exit 1
fi
python3 "$REPO_ROOT/tools/build-seo.py"
if ! grep -q 'sitemap-seo\.xml' "$DOCS/sitemap-index.xml"; then
  echo "build-site: build-seo.py ran but sitemap-seo.xml is not in the sitemap index." >&2
  exit 1
fi
